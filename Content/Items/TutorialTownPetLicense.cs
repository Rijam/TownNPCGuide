		...
		public override bool? UseItem(Player player) {
			...
			if (player.whoAmI == Main.myPlayer && player.itemAnimation > 0) {
				...
				if (!TownNPCGuideWorld.boughtTutorialTownPet || NPC.AnyNPCs(npcType)) {
					TutorialTownPetUnlockOrExchangePet(ref TownNPCGuideWorld.boughtTutorialTownPet, npcType, this.GetLocalizationKey("LicenseExampleTownPetUse")); // Modified NPC.UnlockOrExchangePet method.
				}
				return true;
			}
			return false;
